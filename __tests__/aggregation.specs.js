...
describe('aggregation', () => {
  ...
  async function insertAggregationData(key = TIMESERIES_KEY) {
    ...
    for (let i = 10; i < 50; i++) {
      ...
      data.push([i, val]);
      calls.push( () => client.add(key, i, 'value', val) );
    }
  ...
  async function runAggregation(key, min, max, aggType) {
    ...
    const response = await client.range(key, min, max, options);
  ...
  test('stats', async () => {
    ...
    const response = await client.range(TIMESERIES_KEY, 10, 50, options);
  ...
  test('distinct', async () => {
    ...
    const response = await client.range(TIMESERIES_KEY, '-', '+', options);
